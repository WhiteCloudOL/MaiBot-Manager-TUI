...
impl App {
    ...
    fn dashboard_action_lines(
        ...
        match tab {
            ...
            DashboardTab::Protocol => {
                if let Some(card) = selected {
                    actions.push(format!("Enter 查看 {} 的限制说明", card.title));
                } else {
                    actions.push("Enter 查看协议端状态与限制".to_string());
                }
            ...
            DashboardTab::Access => {
                if let Some(card) = selected {
                    match card.id {
                        ...
                        "access-note" => {
                            actions.push("Enter 查看当前 macOS 访问能力边界".to_string());
    ...
    fn dashboard_context_hint(&self, tab: DashboardTab, focus: DashboardFocus) -> String {
        ...
        match tab {
            ...
            DashboardTab::Protocol => "↑/↓ 选择协议端  Enter 查看限制  Tab 回标签".to_string(),
