...
impl App {
    ...
    pub(crate) fn show_access_info(&self) -> Result<()> {
        ...
        if napcat_cfg.exists() {
            ...
            self.print_kv(
                "地址",
                &format!("http://{}:{}", public_ip, data["port"].as_i64().unwrap_or(0)),
            );
    ...
    pub(crate) fn initialize_maibot_access_config(&self) -> Result<()> {
        ...
        self.print_hint("注意：监听 0.0.0.0 会让 WebUI 暴露在外部网络，请确认已设置访问令牌或防火墙规则。");
        self.print_line();
    ...
    pub(crate) fn modify_adapter_config(&self) -> Result<()> {
        let cfg = self.require_config()?;
        let path = PathBuf::from(cfg.mai_path).join("MaiBot/plugins/MaiBot-Napcat-Adapter/config.toml");
        if !path.exists() {
        ...
        loop {
            ...
            self.print_kv("群聊模式", doc["group_list_type"].as_str().unwrap_or("Unknown"));
            self.print_kv(
                "群聊列表",
                &doc["group_list"].as_array().map(display_array).unwrap_or_default(),
            );
            self.print_kv("私聊模式", doc["private_list_type"].as_str().unwrap_or("Unknown"));
            self.print_kv(
                "私聊列表",
                &doc["private_list"].as_array().map(display_array).unwrap_or_default(),
            );
            self.print_kv(
                "封禁 QQ",
                &doc["ban_user_id"].as_array().map(display_array).unwrap_or_default(),
            );
            let choice = Select::with_theme(&self.theme)
            ...
            match choice {
                ...
                1 => modify_numeric_array(&mut doc, "group_list", true, &self.theme)?,
                2 => modify_numeric_array(&mut doc, "group_list", false, &self.theme)?,
                3 => toggle_string(&mut doc, "private_list_type", "whitelist", "blacklist"),
                4 => modify_numeric_array(&mut doc, "private_list", true, &self.theme)?,
                5 => modify_numeric_array(&mut doc, "private_list", false, &self.theme)?,
                6 => modify_numeric_array(&mut doc, "ban_user_id", true, &self.theme)?,
                7 => modify_numeric_array(&mut doc, "ban_user_id", false, &self.theme)?,
                _ => break,
            }
