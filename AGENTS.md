...
**CLI contract.** CLI commands should reuse the same `App` action methods used by TUI menus rather than duplicating shell strings. `maibot install` / `maibot update` build an `InstallPlan` from existing config plus command-line overrides and then call `run_install`. CLI may ask for confirmation at risk points, but every install/update prompt that blocks scripting must have an explicit CLI strategy flag that bypasses the prompt: `--github-fallback`, `--git-dirty`, `--napcat-conflict`, `--llbot-update`, `--snowluma-swap`, and `--snowluma-conflict`. `maibot access init` prompts by default and `--yes` bypasses it. `maibot core|napcat|llbot|snowluma ...` should remain script-friendly: status/log commands print and exit, while interactive commands (`core exec`, `llbot exec`, `napcat exec`, `snowluma exec`) intentionally inherit stdio.

**Plugin directory naming.** Plugin install/update must not assume the final directory name equals the repository name. After clone/update, resolve the plugin's `_manifest.json` and use its `id` with every `.` replaced by `_` as the canonical folder name under `MaiBot/plugins` (for example, `maibot-team.snowluma-adapter` becomes `maibot-team_snowluma-adapter`). `src/plugins.rs` owns this logic for generic plugin installs, and built-in NapCat / SnowLuma adapter installs in `installer.rs` must follow the same rule. Keep compatibility with the historical `MaiBot/plugins/MaiBot-Napcat-Adapter` path by migrating it to `maibot-team_napcat-adapter`; if the destination already exists, preserve the old directory as a backup instead of overwriting it.
...
**Install risk strategy fields.** `InstallPlan` carries prompt-bypass strategy fields that are not persisted to `~/.maibot_config`: `github_fallback`, `git_dirty_mode`, `napcat_conflict_mode`, `llbot_update_mode`, `snowluma_swap_mode`, and `snowluma_conflict_mode`. Defaults are interactive for TUI and CLI unless a CLI flag overrides them. MaiBot main-repo updates are special-cased so a single local `uv.lock` change is automatically discarded before fetch; all other dirty Git states use the selected strategy or prompt.
...
**Config.** `~/.maibot_config` is a shell-style `KEY="value"` file (NOT TOML), read/written by `runtime.rs::{load_config,save_config}`. It persists the selected PyPI source through `PIP_DISPLAY`, `PIP_INDEX`, and `PIP_HOST`; install plans must restore and save these fields. When a source is configured, core startup must apply it to both uv and virtualenv launches using runtime environment variables; uv must set `UV_NO_CONFIG=1` so the selected source takes precedence over project configuration. `require_config()` is the gate every management menu uses to refuse to run before installation.
...
- TUI task failures must never terminate the manager. Propagate operation errors out of legacy menus and dashboard action handlers to the platform dashboard loop; it must reset to the Overview/sidebar state and show a centered, high-visibility `DashboardPopup` with the primary cause and a short cause chain. Keep CLI behavior unchanged: it prints the error and exits nonzero.
