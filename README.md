...
> 未指定的安装参数优先从 `~/.maibot_config` 读取，无历史记录则采用推荐默认值（`~/maimai` 目录、`uv` 环境、`NapCatQQ` 协议端）。
> *特殊逻辑*：如果 MaiBot 主仓库只有 `uv.lock` 一个文件被修改，程序会自动丢弃该锁文件改动继续同步上游；其他本地改动则按 `--git-dirty` 策略处理。

### 2. MaiBot 核心管理

```bash
maibot core start        # 后台启动 MaiBot，screen 会话名为 maibot
maibot core start --exec # 启动后立即进入 screen 控制台（首次运行排错推荐）
maibot core stop         # 停止 maibot screen 会话
maibot core restart      # 重启 MaiBot 核心
maibot core status       # 输出 running / stopped，适合脚本判断
maibot core logs         # 查看最近 100 行 screen 缓冲日志 (hardcopy 读取，不抢占)
maibot core logs --tail 200     # 指定输出行数
maibot core logs --follow       # 每 2 秒刷新 hardcopy 输出，不附着 screen
maibot core exec         # 进入 screen 控制台；退出请按 Ctrl+A 再按 D
...
maibot access init              # 绑定 WebUI 到 0.0.0.0 并启用 Napcat Adapter（需交互确认）
...
*(可修改模块：安装目录 / 安装模式 / Python 环境 / 虚拟环境处理 / GitHub 线路 / PyPI 源 / Bot 协议端 / Docker 镜像)*
